...
myfile.write("reg [`DATA_LENGTH-1:0] median;")
...
if (W%2 ==0):
    myfile.write("\t\tif (flag ==0)\n\t\t\t median <= (dataOut%d + dataOut%d)>>1 ;\n\tend\nend\n" %(int(math.ceil(W/2.0)),int(math.ceil(W/2.0))+1))
else:
    myfile.write("\t\tif (flag ==0)\n\t\t\t median <= dataOut%d ;\n\tend\nend\n" %int(math.ceil(W/2.0)) )
...
myfile.write("medianFilterCell  m1(.X(X) ,.oldData(oldData) ,.flag(flag),.reset(reset) ,.clk(clk) ,.R_in(0) ,.dataR_in(dataOut2) ,.dataL_in(0) \n\t ,.R_out(R1) ,.data_out(dataOut1)  );\n" )
for i in range(2,W):
    myfile.write("medianFilterCell  m%d(.X(X) ,.oldData(oldData)  ,.flag(flag),.reset(reset) ,.clk(clk) ,.R_in(R%d) ,.dataR_in(dataOut%d) ,.dataL_in(dataOut%d) \n\t ,.R_out(R%d) ,.data_out(dataOut%d)  );\n"%(i,i-1,i+1,i-1,i,i))
myfile.write("medianFilterCell  m%d(.X(X) ,.oldData(oldData) ,.flag(flag),.reset(reset) ,.clk(clk) ,.R_in(R%d),.dataR_in(0) ,.dataL_in(dataOut%d) \n\t ,.R_out(R%d)  ,.data_out(dataOut%d));\n"%(W ,W-1  ,W-1 ,W  ,W))
